...
def add_salt_and_pepper_noise(image, amount=0.05):
    ...
    if len(np_image.shape) == 2:
        ...
        row, col, channels = np_image.shape

    num_salt = np.ceil(amount * np_image.size * 0.5)
    num_pepper = np.ceil(amount * np_image.size * 0.5)

    coords = [np.random.randint(0, i - 1, int(num_salt)) for i in np_image.shape[:2]]
    if channels == 1:
        np_image[coords[0], coords[1]] = 255
    else:
        np_image[coords[0], coords[1], :] = 255

    coords = [np.random.randint(0, i - 1, int(num_pepper)) for i in np_image.shape[:2]]
    if channels == 1:
...
if args.max_num == -1:
    tar_img_path_lst = sorted(Path(args.tar_dir).resolve().glob('*.tiff'))[args.start_idx:]
else:
    tar_img_path_lst = sorted(Path(args.tar_dir).resolve().glob('*.tiff'))[args.start_idx: args.start_idx + args.max_num]
...
if args.if_psnr or args.if_ssim or args.if_msssim or args.if_lpips:
    ...
    for idx_tar, tar_img_path in enumerate(tar_img_path_lst):
        ...
        if args.if_psnr:
            info_dict['psnr_tar'] = [PSNR().forward(tar_img, src_path)]
            if args.if_dst:
                info_dict['psnr_dst'] = [PSNR().forward(dst_img, src_path)]
